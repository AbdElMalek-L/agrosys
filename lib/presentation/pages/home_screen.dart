...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _refreshSignalStrength() async {
    ...
    final selectedIndex = context.read<AppStateCubit>().state.selectedDeviceIndex;
  ...
  void _updateSignalStrength(String message) {
    ...
    final signalMatch = RegExp(r'\b([0-9]|[12][0-9]|3[01])\b').firstMatch(message);
    if (signalMatch != null) {
    ...
    if (senderClean.contains(selectedDeviceNumber)) {
      if (_lastSms != newMessage) {
        ...
        if (newMessage != null) {
          ...
          if (newMessage.startsWith("Relay ON!") || newMessage.startsWith("Relay OFF!")) {
            ...
            if (newMessage.contains(selectedDeviceNumber)) {
              print('Updating power state to: ${isOn ? "ON" : "OFF"}'); // Debug log
              context.read<DeviceCubit>().updatePowerState(selectedIndex, isOn);
              _isWaitingForConfirmation = false;

              // Add activity to recent activities
              final activity = Activity.fromSms(newMessage, latest.date ?? DateTime.now());
        ...
        if (_initialCheckDone && mounted) {
          ...
          final isSignalMessage = RegExp(r'^\s*([0-9]|[12][0-9]|3[01])\s*$').hasMatch(newMessage ?? '');
          final isRelayStatus = newMessage?.startsWith("Relay ON!") == true || newMessage?.startsWith("Relay OFF!") == true;
                      ...
                      children: [
                        GestureDetector(
                          onVerticalDragEnd: (details) {
                            if (details.primaryVelocity! > 0) { // Swipe down
                              ...
                              ElevatedButton.icon(
                                onPressed: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => DeviceSmsHistoryPage(
                                        device: devices[appState.selectedDeviceIndex],
                                      ),
                                    ),
                                  );
